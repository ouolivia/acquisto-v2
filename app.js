...
function bind(){
  ...
  document.querySelectorAll('[data-edit-model]').forEach(x=>x.onclick=()=>{const lines=getBatch().lines.filter(l=>l.model===x.dataset.editModel),first=lines[0];if(!first)return;draft={model:first.model,cost:draftPrice(first.cost),sale:draftPrice(first.sale),unit:first.unit,packSize:first.unit==='pack'?String(first.packSize):'',qty:String(first.qty),note:first.note||'',colors:[...new Set(lines.map(l=>l.color).filter(Boolean))],stores:[...new Set(lines.map(l=>l.store).filter(n=>STORES.includes(n)))].sort((a,b)=>a-b),editIds:lines.map(l=>l.id),editContext:'model'};screen='entry';render();});
  document.querySelectorAll('[data-edit-preview-store]').forEach(x=>x.onclick=()=>{const store=Number(x.dataset.editPreviewStore),lines=getBatch().lines.filter(l=>l.model===draft.model&&l.store===store),first=lines[0];if(!first)return;draft={model:first.model,cost:draftPrice(first.cost),sale:draftPrice(first.sale),unit:first.unit,packSize:first.unit==='pack'?String(first.packSize):'',qty:String(first.qty),note:first.note||'',colors:[...new Set(lines.map(l=>l.color).filter(Boolean))],stores:[store],editIds:lines.map(l=>l.id),editContext:'preview'};render();requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'smooth'}));toast(`正在修改 ${store} 店`);});
...
function action(name){
  ...
  if(name==='finish-model'){syncDraft();if(!draft.model)return toast('当前还没有输入型号');const modelLines=getBatch().lines.filter(l=>l.model===draft.model);if(!modelLines.length)return toast('请先分配当前型号');if(modelLines.some(l=>pricePending(l.cost)||pricePending(l.sale))&&!confirm('进价或卖价尚未填写，将在明细中显示“待定”。是否确定输入下一个款式？'))return;draft=freshDraft();render();setTimeout(()=>document.querySelector('#model')?.focus(),0);toast('可以输入下一个型号');}
