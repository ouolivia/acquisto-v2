...
function entryView(){
  ...
  return `${header('采购录入',`${b.supplier} · ${b.date}`)}<div class="wrap">
    ...
    <section class="card"><div class="section-head"><h2>数量与门店</h2><button class="link-btn" data-action="toggle-stores">${draft.stores.filter(n=>STORES.includes(n)).length===STORES.length?'取消全选':'全选'}</button></div>
      <div class="qty-allocate-row"><div class="qty-input-wrap"><input id="qty" type="${draft.unit==='pack'?'text':'number'}" ${draft.unit==='pack'?'inputmode="decimal"':'min="1" step="1" inputmode="numeric"'} value="${esc(draft.qty)}" placeholder="${draft.unit==='pack'?'数量或半':'数量'}"><span>${draft.unit==='pack'?'包':'件'}</span></div><button class="btn btn-primary" data-action="allocate">${draft.editIds.length?'保存修改':'分配到所选门店'}</button></div>
